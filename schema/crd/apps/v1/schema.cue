...
import (
	"cue.dev/x/k8s.io/apimachinery/pkg/runtime"
...
// can not be updated. The API Server will fail validation of all
// requests that attempt to mutate the Data field.
// ControllerRevisions may, however, be deleted. Note that, due
// to its use by both the DaemonSet and StatefulSet controllers
// for update and rollback, this object is beta. However, it may
// be subject to name and representation changes in future
...
#DaemonSet: {
	...
	// date by some window of time. Populated by the system.
	// Read-only. More info:
	// https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
...
#DaemonSetSpec: {
	...
	// The number of old history to retain to allow rollback. This is
...
#DaemonSetStatus: {
	...
	"currentNumberScheduled"!: int32 & int
	...
	// https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
	"desiredNumberScheduled"!: int32 & int

	// The number of nodes that should be running the daemon pod and
	// have one or more of the daemon pod running and available
	// (ready for at least spec.minReadySeconds)
	"numberAvailable"?: int32 & int

	// The number of nodes that are running the daemon pod, but are
	// not supposed to run the daemon pod. More info:
	// https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
	"numberMisscheduled"!: int32 & int

	// numberReady is the number of nodes that should be running the
	// daemon pod and have one or more of the daemon pod running with
	// a Ready Condition.
	"numberReady"!: int32 & int

	// The number of nodes that should be running the daemon pod and
	// have none of the daemon pod running and available (ready for
	// at least spec.minReadySeconds)
	"numberUnavailable"?: int32 & int
	...
	"updatedNumberScheduled"?: int32 & int
}
...
#DeploymentSpec: {
	...
	// before it is considered to be failed. The deployment
	// controller will continue to process failed deployments and a
	// condition with a ProgressDeadlineExceeded reason will be
	// surfaced in the deployment status. Note that progress will not
	// be estimated during the time a deployment is paused. Defaults
	// to 600s.
	...
	// The number of old ReplicaSets to retain to allow rollback. This
...
#DeploymentStatus: {
	...
	// This is the total number of pods that are still required for
	// the deployment to have 100% available capacity. They may
	// either be pods that are running but not yet available or pods
...
#ReplicaSet: {
	...
	// This data may be out of date by some window of time. Populated
...
#ReplicaSetSpec: {
	...
	// replica count. Label keys and values that must match in order
...
#ReplicaSetStatus: {
	...
	// readyReplicas is the number of pods targeted by this ReplicaSet
	// with a Ready Condition.
	"readyReplicas"?: int32 & int
...
#RollingUpdateDaemonSet: {
	...
	// during an update. Value can be an absolute number (ex: 5) or a
	// percentage of desired pods (ex: 10%). This can not be 0 if
	// MaxUnavailable is 0. Absolute number is calculated from
	// percentage by rounding up to a minimum of 1. Default value is
	// 0. Example: when this is set to 30%, at most 30% of the total
	// number of nodes that should be running the daemon pod (i.e.
	// status.desiredNumberScheduled) can have their a new pod
	// created before the old pod is marked as deleted. The update
	// starts by launching new pods on 30% of nodes. Once an updated
	// pod is available (Ready for at least minReadySeconds) the old
	// DaemonSet pod on that node is marked deleted. If the old pod
	// becomes unavailable for any reason (Ready transitions to
	// false, is evicted, or is drained) an updated pod is
	// immediatedly created on that node without considering surge
	// limits. Allowing surge implies the possibility that the
	// resources consumed by the daemonset on any given node can
	// double if the readiness check fails, and so resource intensive
	// daemonsets should take into account that they may cause
	...
	// a percentage of total number of DaemonSet pods at the start of
	// the update (ex: 10%). Absolute number is calculated from
	// percentage by rounding up. This cannot be 0 if MaxSurge is 0
	// Default value is 1. Example: when this is set to 30%, at most
	// 30% of the total number of nodes that should be running the
	// daemon pod (i.e. status.desiredNumberScheduled) can have their
	// pods stopped for an update at any given time. The update
	// starts by stopping at most 30% of those DaemonSet pods and
	// then brings up new DaemonSet pods in their place. Once the new
...
#RollingUpdateDeployment: {
	...
	// percentage by rounding up. Defaults to 25%. Example: when this
	// is set to 30%, the new ReplicaSet can be scaled up immediately
	// when the rolling update starts, such that the total number of
	// old and new pods do not exceed 130% of desired pods. Once old
	// pods have been killed, new ReplicaSet can be scaled up
	// further, ensuring that total number of pods running at any
	// time during the update is at most 130% of desired pods.
	"maxSurge"?: intstr.#IntOrString

	// The maximum number of pods that can be unavailable during the
	// update. Value can be an absolute number (ex: 5) or a
	// percentage of desired pods (ex: 10%). Absolute number is
	// calculated from percentage by rounding down. This can not be 0
	// if MaxSurge is 0. Defaults to 25%. Example: when this is set
	// to 30%, the old ReplicaSet can be scaled down to 70% of
	// desired pods immediately when the rolling update starts. Once
	// new pods are ready, old ReplicaSet can be scaled down further,
	// followed by scaling up the new ReplicaSet, ensuring that the
	// total number of pods available at all times during the update
	// is at least 70% of desired pods.
	"maxUnavailable"?: intstr.#IntOrString
}
...
#RollingUpdateStatefulSetStrategy: {
	...
	// update. Value can be an absolute number (ex: 5) or a
	// percentage of desired pods (ex: 10%). Absolute number is
	// calculated from percentage by rounding up. This can not be 0.
	// Defaults to 1. This field is alpha-level and is only honored
	// by servers that enable the MaxUnavailableStatefulSet feature.
	// The field applies to all pods in the range 0 to Replicas-1.
	// That means if there is any unavailable pod in the range 0 to
	// Replicas-1, it will be counted towards MaxUnavailable.
	"maxUnavailable"?: intstr.#IntOrString

	// Partition indicates the ordinal at which the StatefulSet should
	// be partitioned for updates. During a rolling update, all pods
	// from ordinal Replicas-1 to Partition are updated. All pods
	// from ordinal Partition-1 to 0 remain untouched. This is
	// helpful in being able to do a canary based deployment. The
	// default value is 0.
	"partition"?: int32 & int
}
...
#StatefulSetOrdinals: {
	// start is the number representing the first replica's index. It
	// may be used to number replicas from an alternate index (eg:
	// 1-indexed) over the default 0-indexed names, or to orchestrate
	// progressive movement of replicas from one StatefulSet to
	// another. If set, replica indices will be in the range:
...
#StatefulSetSpec: {
	...
	// StatefulSet. The default ordinals behavior assigns a "0" index
	// to the first replica and increments the index by one for each
	// additional replica requested.
	"ordinals"?: #StatefulSetOrdinals

	// persistentVolumeClaimRetentionPolicy describes the lifecycle of
	// persistent volume claims created from volumeClaimTemplates. By
	// default, all persistent volume claims are created as needed
	// and retained until manually deleted. This policy allows the
	// lifecycle to be altered, for example by deleting persistent
	// volume claims when their stateful set is deleted, or when
	// their pod is scaled down.
	...
	// the controller will wait until each pod is ready before
	// continuing. When scaling down, the pods are removed in the
	// opposite order. The alternative policy is `Parallel` which
	// will create pods in parallel to match the desired scale
	// without waiting, and on scale down will delete all pods at
	// once.
	...
	// will be maintained in the StatefulSet's revision history. The
	// revision history consists of all revisions not represented by
	// a currently applied StatefulSetSpec version. The default value
	// is 10.
	"revisionHistoryLimit"?: int32 & int
	...
	// have a unique identity from the rest of the StatefulSet. Each
	// pod will be named with the format
	// <statefulsetname>-<podindex>. For example, a pod in a
	// StatefulSet named "web" with index number "3" would be named
	// "web-3". The only allowed template.spec.restartPolicy value is
	// "Always".
	"template"!: v1_9.#PodTemplateSpec
	...
	// volumeClaimTemplates is a list of claims that pods are allowed
	// to reference. The StatefulSet controller is responsible for
	// mapping network identities to claims in a way that maintains
	...
	"volumeClaimTemplates"?: [...v1_9.#PersistentVolumeClaim]
}
...
#StatefulSetStatus: {
	...
	// this StatefulSet. It corresponds to the StatefulSet's
